...
// Autossuficiente de propósito — sem imports locais — porque um import
// relativo para api/_shared/gemini.ts causava ERR_MODULE_NOT_FOUND no
// runtime de função da Vercel (mesmo funcionando no Vite em dev/preview).

import { GoogleGenAI } from '@google/genai';

const AUTHOR_NAME = 'Daniel Santos da Silva';
...
export default async function handler(req: any, res: any) {
  ...
  const prompt = language === 'en' ? buildPromptEn(project) : buildPromptPt(project);
